...
import 'package:brothers88/crew/detail_order.dart';
import 'package:brothers88/login_page.dart';
import 'package:brothers88/logout.dart';
import 'package:flutter/material.dart';
import 'package:http/http.dart' as http;
import 'package:shared_preferences/shared_preferences.dart';

class Crew extends StatefulWidget {
  Crew({Key? key}) : super(key: key);
                    ...
                    builder: (context, snapshot) {
                      if (snapshot.hasData) {
                        ...
                        if (data?.length == 0) {
                          print(data?.length);
                          return Center(
                          child: Text("Tidak ada data yang perlu di validasi"),
                        );
                        } else {
...
Future<List<Order>> fetchData() async {
  ...
  final response = await http
      .get(Uri.parse("https://api.brothers-88.com/crew/order/list"), headers: {
    "Accept": "application/json",
    "Access-Control-Allow-Origin": "*",
    "Authorization": 'Bearer $token',
  });
